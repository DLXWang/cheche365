...
public class AdminDaoAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
    ...
    public AdminDaoAuthenticationProvider() {
        setPasswordEncoder(new AdminPasswordEncode());
    }
    ...
    protected void additionalAuthenticationChecks(UserDetails userDetails,
        ...
        // 检查是否超出错误次数
        Object t = stringRedisTemplate.opsForHash().get(ManageCommonConstants.USER_LOCK_KEY, userDetails.getUsername());
        Integer count = t == null ? ManageCommonConstants.PASSWORD_ERROR_COUNT : Integer.valueOf(t.toString());
        if (count == 0) {
            // 用户已锁定
...
//                "Bad credentials"));
            // 记录登录错误次数
            count -= 1;
            stringRedisTemplate.opsForHash().put(ManageCommonConstants.USER_LOCK_KEY, userDetails.getUsername(), count.toString());
            ...
            stringRedisTemplate.expireAt(ManageCommonConstants.USER_LOCK_KEY, calendar.getTime());
            if (count == 0) {
                throw new NoPermissionLoginException(ManageCommonConstants.AUTHENTICATION_LOCKED_CODE);
            } else {
            ...
            stringRedisTemplate.opsForHash().delete(ManageCommonConstants.USER_LOCK_KEY, userDetails.getUsername());
        }

        if (!checkSafePassword(presentedPassword)) {
            stringRedisTemplate.opsForSet().add(ManageCommonConstants.RESET_PASSWORD_KEY, authentication.getPrincipal().toString());
        }
    }
