...
CUI.grid.build=function(rowNum){
    ...
    $.each(CUI.grid.columns,function(i,column){
        ...
        if(!common.validation.isEmpty(index)){
        ...
        if(column.renderer){
            ...
            if(!common.validation.isEmpty(customResult)){
...
CUI.grid.check=function(){
    if (CUI.grid.store == null) {
        ...
        popup.mould.popTipsMould("查询错误", popup.mould.first, popup.mould.warning, "", "57%", null);
        return false;
        ...
        CUI.grid.dom.empty();
        popup.mould.popTipsMould("未检索到符合条件的数据", popup.mould.first, popup.mould.warning, "", "57%", null);
        return false;
    }else{
        if(CUI.grid.result){
...
function createPrefix(obj){
    ...
    if(!common.validation.isEmpty(obj.id)){
        prefix+=TAG.SPACE+PROPERTY.ID+"'"+obj.id+"'"+TAG.SPACE;
    }
    if(!common.validation.isEmpty(obj.name)){
        prefix+=TAG.SPACE+PROPERTY.NAME+"'"+obj.name+"'"+TAG.SPACE;
    }
    if(!common.validation.isEmpty(obj.class)){
        prefix+=TAG.SPACE+PROPERTY.CLASS+obj.class+TAG.SPACE;
    }
    if(!common.validation.isEmpty(obj.style)){
...
function createContent(content,required){
    if(required==true){
        return common.tools.checkToEmpty(content);
    }
    return content;
}
