...
var modify_password = {
    init: function() {
        var content =
            ...
            "</form>"+
            "</div>";
        popup.pop.popInput(content, popup.mould.first, "500px", "375px", "50%", "55%");
        ...
        window.parent.$("#update_button").unbind("click").bind({
            click : function(){
                ...
                if(!common.validation.isPassword(newPassword)){
                    $("#errorsText").text("密码由6到12位大小写字母数字下划线组成！");
                    $("#newPassword").focus();
                    $("#errorsText").parent().parent().show();
                    return false;
                }
                if(!common.validation.isPasswordEx(newPassword)){
                    $("#errorsText").text("密码必须包含大小写字母和数字！");
                    $("#newPassword").focus();
                    $("#errorsText").parent().parent().show();
                    return false;
                }
                //if(!common.validation.isPassword(confirmPassword) || !common.validation.isPasswordEx(confirmPassword)){
                ...
                modify_password.modifyPassword(newPassword);
            }
        });
    ...
    modifyPassword : function(newPassword) {
        ...
        common.ajax.getByAjax(true, "post", "json", "/admin/internalUser/modifyPassword", {id:id,password:newPassword},
            function(data){
                if(data.pass){
