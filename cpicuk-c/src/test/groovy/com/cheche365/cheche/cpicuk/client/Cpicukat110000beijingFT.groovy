...
@Slf4j
class Cpicukat110000beijingFT extends AParserServiceFT {
    ...
    'ID：#id 、DESC：#desc ，用车牌号：#licensePlateNo 、车架号：#vinNo 、引擎号：#engineNo 、车主：#owner 、身份证：#identity 、套餐选项：#packageOptions 、预期报价：#expectedQuoteRecord 测试投保接口，且核保成功'() {
        ...
        if (notSkipThisCase) {
            ...
            def address = new Address(street: '胡同', province: '北京', city: '北京')
            def user = new User(name: '周杰伦',mobile: '131111111111')
            def po = new PurchaseOrder(orderNo: uuid() as String, deliveryAddress: address,applicant: user)
