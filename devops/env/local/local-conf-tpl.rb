...
# the lines below:
#
if File.exists?('cloud-conf') && ARGV[0].eql?('up')
  require 'open-uri'
  require 'yaml'

  token = open($new_discovery_url).read

  data = YAML.load(IO.readlines('cloud-conf')[1..-1].join)
  ...
  yaml = YAML.dump(data)
  File.open('cloud-conf', 'w') { |file| file.write("#cloud-config\n\n#{yaml}") }
end
...
# these options, first copy this file to "local-conf". Then simply
