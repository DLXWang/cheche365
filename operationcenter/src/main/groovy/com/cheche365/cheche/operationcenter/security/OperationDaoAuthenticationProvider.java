 ...
 */
public class OperationDaoAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
    ...
    public OperationDaoAuthenticationProvider() {
        setPasswordEncoder(new OperationCenterPasswordEncode());
    }
    ...
    protected void additionalAuthenticationChecks(UserDetails userDetails,
        ...
        // 检查是否超出错误次数
        Object t = stringRedisTemplate.opsForHash().get(OperationCenterConstants.USER_LOCK_KEY, userDetails.getUsername());
        Integer count = t == null ? OperationCenterConstants.PASSWORD_ERROR_COUNT : Integer.valueOf(t.toString());
        if (count == 0) {
            // 用户已锁定
...
//                "Bad credentials"));
            // 记录登录错误次数
            count -= 1;
            stringRedisTemplate.opsForHash().put(OperationCenterConstants.USER_LOCK_KEY, userDetails.getUsername(), count.toString());
            ...
            stringRedisTemplate.expireAt(OperationCenterConstants.USER_LOCK_KEY, calendar.getTime());
            if (count == 0) {
                throw new NoPermissionLoginException(OperationCenterConstants.AUTHENTICATION_LOCKED_CODE);
            } else {
            ...
            stringRedisTemplate.opsForHash().delete(OperationCenterConstants.USER_LOCK_KEY, userDetails.getUsername());
        }

        // 校验密码安全强度，设置是否重置密码
        if (!checkSafePassword(presentedPassword)) {
            stringRedisTemplate.opsForSet().add(OperationCenterConstants.RESET_PASSWORD_KEY, authentication.getPrincipal().toString());
        }
    }
