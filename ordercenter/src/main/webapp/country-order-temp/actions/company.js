...
export function getUsedAreas(text) {
	return dispatch => $.ajax({
			'type':'GET',
			'url': '/orderCenter/nationwide/areaContactInfo/allArea',
...
export function getUsedInsurances(text) {
	return dispatch => $.ajax({
			'type':'GET',
			'url': '/orderCenter/resource/insuranceCompany/getQuotableCompanies',
			'contentType': 'application/json',
			'dataType':'json',
			'success': function(data){
				let array = []
...
export function changeCompanyStatus(company) { //这个actions 目前没有对应的reducers
	...
	return dispatch => $.ajax({
			'type':'PUT',
			'url': '/orderCenter/nationwide/institutionTemp/' + company.id + '/' + status,
			'contentType': 'application/json',
			'dataType':'json',
			'success': function(array){
