		...
		for (let item in COMPANYKEYS.baseInfo) {
			switch(item) {
			    ...
			    case 'comment':
			        ...
			        break;
			    case 'contactName':
			        items[item] = isFlag ? <input className='item input' type='text' maxLength='20' defaultValue={this.props.company[item]}
                        name={item} ref={item} /> : <div className='item text-show height-27'>{this.props.company[item]}</div>;
			        break;
			    case 'contactMobile':
			        items[item] = isFlag ? <input className='item input' type='text' defaultValue={this.props.company[item]} maxLength='11' pattern="^((13|15|17|18)[0-9]{9})|(0(([1-9]\d)|([3-9]\d{2}))(-?)((\d{7})|(\d{8})))$"
                                                  name={item} ref={item} />: <div className='item text-show height-27'>{this.props.company[item]}</div>;
			        break;
		        case 'contactEmail':
