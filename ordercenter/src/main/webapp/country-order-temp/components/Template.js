...
export default class Template extends Component {
	...
	_handleCancelTemplate(){
  	...
  	_handleSubmitTemplate() {
        ...
        if(this.props.companyObj.company.insurances.length == 0) {
            this.props._toggleMessageDialog("请选择保险公司");
            return;
        }
		...
		$.ajax({
			'type':'POST',
			'url':'/orderCenter/nationwide/institutionTemp',
			'data':JSON.stringify(institution),
			'contentType':'application/json',
			'success':function(result){
                if(result.pass) {
                    this.props._toggleMessageDialog('出单机构保存成功', true);
	...
	render() {
        ...
        let titleContent = companyObj.company.id ? (companyObj.isEditCompany ? "编辑出单机构" : "查看出单机构") : "新建出单机构";
	    templateActions.push(<FlatButton
	        key={2}
	        label="取消"
	        primary={true}
