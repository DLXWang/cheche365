...
def identify = { hostname ->
    productionEnv ?
        "/data/nfs0/logs/piccuk-ms/$hostname/$port"           // nfs0
        : (('/data/nfs2' as File).exists() ?
            "/data/nfs2/logs/piccuk-ms/$hostname/$port"       // nfs2
            : 'logs')                                      // local
}
...
['org.springframework.cloud', 'org.eclipse.jetty'].each { name ->
    logger name, INFO, ['piccukmsDailyRollingFile']
}
