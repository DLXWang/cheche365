...
public class InMessage {
    ...
    private String MsgType = "text";//消息类型，event
    private Long MsgId;
    // 文本消息
    ...
    // 事件
    private String Event;//事件类型，subscribe(订阅)、unsubscribe(取消订阅)、SCAN(关注)、LOCATION(上报地理位置)、CLICK(点击自定义菜单)、VIEW(点击菜单跳转链接)
    private String EventKey;//事件KEY值，（subscribe）qrscene_为前缀，后面为二维码的参数值、（SCAN）是一个32位无符号整数，即创建二维码时的二维码scene_id、（CLICK）与自定义菜单接口中KEY值对应、(VIEW)设置的跳转URL
    ...
    private Logger logger = LoggerFactory.getLogger(InMessage.class);
    ...
    public static InMessage parseMessage(String message){
        return XStreamUtil.parse(message,InMessage.class);
    }
}
